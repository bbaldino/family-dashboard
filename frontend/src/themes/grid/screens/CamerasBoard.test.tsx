...
const DEFAULT_URL = camerasIntegration.schema.parse({}).doorbell_live_url
...
describe('CamerasBoard', () => {
  ...
  it('shows the configured camera page', async () => {
    stubConfig({ 'cameras.doorbell_live_url': 'https://cam.test/front' })
  ...
  it('asks for the camera url to be configured when it is blanked out', async () => {
    stubConfig({ 'cameras.doorbell_live_url': '' })
    renderBoard(newClient())
  ...
  it('picks up a camera url change with no remount', async () => {
    const fetchMock = stubConfig({ 'cameras.doorbell_live_url': 'https://cam.test/old' })
    ...
    fetchMock.mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ 'cameras.doorbell_live_url': 'https://cam.test/new' }),
    })
    await act(async () => {
  ...
  // (see `integrations/cameras/config.test.ts`); the schema coerces them now,
