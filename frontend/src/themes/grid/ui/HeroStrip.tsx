  ...
  return (
    <div className="bg-bg-card rounded-[var(--radius-card)] shadow-[var(--shadow-card)] flex items-center gap-5 px-7 py-3">
      ...
      <div className="flex-1">
        <div className="text-[10px] font-bold uppercase tracking-[0.5px] text-palette-1 mb-1">
          {label}
        </div>
        {events.length === 0 ? (
            ...
            {events.slice(0, 2).map((event, i) => (
              <div key={i} className={`${i > 0 ? 'pl-3 border-l border-separator' : ''}`}>
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-[13px] font-semibold text-palette-1 whitespace-nowrap flex-shrink-0">
                    {event.time}
                  </div>
                </div>
                {event.driveTag && (
                  <div className="mt-0.5">
                    <DriveTag
                      displayText={event.driveTag.displayText}
                      urgency={event.driveTag.urgency}
                    />
                  </div>
                )}
              </div>
        ...
        <div>
          ...
          <div className="text-[12px] text-text-secondary">
            {weatherHigh && weatherLow ? (
              <>
                <span className="font-medium">{weatherHigh}&deg;</span> /{' '}
                <span className="text-text-muted">{weatherLow}&deg;</span>
              </>
            ) : (
              weatherCondition || ''
            )}
