...
describe('buildShelfActionGroups', () => {
  it('gives a track all four play actions plus both go-to actions', () => {
    const groups = buildShelfActionGroups({ item: track, play: vi.fn(), navigate: vi.fn(), onClose: vi.fn() })
  ...
  it('drops "Play just this track" for an album — tapping the card already plays it as-is', () => {
    const groups = buildShelfActionGroups({ item: album, play: vi.fn(), navigate: vi.fn(), onClose: vi.fn() })
    expect(groups[0].items.map((i) => i.label)).toEqual(['Play radio from this', 'Play next', 'Add to queue'])
  })
  ...
  it('offers three play actions and no go-to actions for an artist', () => {
    const groups = buildShelfActionGroups({ item: artist, play: vi.fn(), navigate: vi.fn(), onClose: vi.fn() })
    expect(groups[0].items.map((i) => i.label)).toEqual(['Play radio from this', 'Play next', 'Add to queue'])
    expect(groups[1].items).toEqual([])
  })

  it('offers three play actions and no go-to actions for a playlist', () => {
    const groups = buildShelfActionGroups({ item: playlist, play: vi.fn(), navigate: vi.fn(), onClose: vi.fn() })
    expect(groups[0].items.map((i) => i.label)).toEqual(['Play radio from this', 'Play next', 'Add to queue'])
    expect(groups[1].items).toEqual([])
  })
  ...
  it('"Play radio from this" plays with radio for a non-track item', () => {
    const play = vi.fn()
    const groups = buildShelfActionGroups({ item: album, play, navigate: vi.fn(), onClose: vi.fn() })
    groups[0].items[0].onSelect()
    expect(play).toHaveBeenCalledWith(album.uri, expect.objectContaining({ radio: true, enqueueMode: 'play' }))
  })

  it('"Play next" and "Add to queue" use the matching enqueue mode', () => {
    const play = vi.fn()
    const groups = buildShelfActionGroups({ item: playlist, play, navigate: vi.fn(), onClose: vi.fn() })
    groups[0].items[1].onSelect()
    groups[0].items[2].onSelect()
    expect(play).toHaveBeenNthCalledWith(1, playlist.uri, expect.objectContaining({ enqueueMode: 'next' }))
    expect(play).toHaveBeenNthCalledWith(2, playlist.uri, expect.objectContaining({ enqueueMode: 'add' }))
  })

  it('"Go to artist" navigates to the URL-encoded artist route', () => {
    const navigate = vi.fn()
    const groups = buildShelfActionGroups({ item: track, play: vi.fn(), navigate, onClose: vi.fn() })
  ...
  it('calls onClose after every action', () => {
    const onClose = vi.fn()
    const groups = buildShelfActionGroups({ item: track, play: vi.fn(), navigate: vi.fn(), onClose })
    groups[0].items[0].onSelect()
