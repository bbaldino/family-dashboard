...
describe('AlbumTrackRow', () => {
  it('shows a mono track number, not the playing glyph, when not playing', () => {
    render(<AlbumTrackRow track={track} index={0} isPlaying={false} isMenuOpen={false} onToggleMenu={vi.fn()} groups={[]} />)
    expect(screen.getByText('01')).toBeInTheDocument()
  })

  it('shows the rust ▸ glyph in place of the number when playing', () => {
    render(<AlbumTrackRow track={track} index={0} isPlaying isMenuOpen={false} onToggleMenu={vi.fn()} groups={[]} />)
  ...
  it('omits the feat. line for a track with only its own artist', () => {
    const soloTrack = { ...track, artists: [{ name: 'The Chemical Brothers', uri: 'fixture://artist/1' }] }
    render(<AlbumTrackRow track={soloTrack} index={0} isPlaying={false} isMenuOpen={false} onToggleMenu={vi.fn()} groups={[]} />)
