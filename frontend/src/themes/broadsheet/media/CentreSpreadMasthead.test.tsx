...
describe('CentreSpreadMasthead', () => {
  it('shows the room, track title, and "Side A · Track {n}" when a track number is present', () => {
    const onClose = vi.fn()
    render(<CentreSpreadMasthead room="Kitchen" trackTitle="Amber Hours" trackNumber={4} onClose={onClose} />)
  ...
  it('drops the "Track {n}" clause — never "of {m}" — when there is no track number', () => {
    const onClose = vi.fn()
    render(<CentreSpreadMasthead room="Kitchen" trackTitle="Amber Hours" trackNumber={null} onClose={onClose} />)
  ...
  it('falls back to a roomless "Now playing" when there is no room', () => {
    const onClose = vi.fn()
    render(<CentreSpreadMasthead room={null} trackTitle="Amber Hours" trackNumber={1} onClose={onClose} />)
    expect(screen.getByText('Now playing')).toBeInTheDocument()
  })

  it('calls onClose when Close is tapped', () => {
    const onClose = vi.fn()
    render(<CentreSpreadMasthead room="Kitchen" trackTitle="Amber Hours" trackNumber={1} onClose={onClose} />)
